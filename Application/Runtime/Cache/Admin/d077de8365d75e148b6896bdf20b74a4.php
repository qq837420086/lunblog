...
$(document).ready(function(){
	$("#loginout").click(function(){
						...
						<ul class="list-group">
							<li class="list-group-item">
								<p class="text-success"><a href="<?php echo U('Index/index');?>" title="返回首页" class="tip-bottom"><i class="fa fa-home"></i> 首页</a> /
									<a href="#" class="tip-bottom"> 本地生活</a> / <a href="<?php echo U('life/index');?>" class="tip-bottom">信息管理</a></p>
							</li>
						</ul>
					</div>
				</div>
		...
		<a href="<?php echo U('Life/add');?>" class="btn btn-primary">添加信息</a>
		<br />
		<br />
		<div class="row">
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-title">
						<h5><i class="fa fa-tasks"></i> 信息管理</h5>
					</div>
						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">										
										...
										<td><img src="/Uploads/<?php echo ($vo["photo"]); ?>" style="width: 120px;height: 60px" /></td>
										<td><?php echo ($vo["mobile"]); ?></td>
										<td><?php echo ($vo["addr"]); ?></td>
										<td><?php echo (date('Y-m-d H:i:s',$vo["create_time"])); ?></td>
										<td><?php echo (date('Y-m-d H:i:s',$vo["last_time"])); ?></td>	
										<td>
											<?php if($vo["audit"] == 1 ): ?>已审核
												<?php else: ?>
						 						未审核<?php endif; ?>
										</td>
										<td>
						    				<?php if($vo["is_red"] == 0 ): ?>否<?php endif; ?>
						    				<?php if($vo["is_red"] == 1 ): ?>推荐<?php endif; ?>
										</td>
										<td>
											<a href="<?php echo U('edit',array('life_id'=>$vo['life_id']));?>" class="btn btn-primary btn-mini">
												<i class="fa fa-pencil"></i> 编辑</a>&nbsp;&nbsp;
											<a onClick="return confirm('是否删除此条记录')" href="<?php echo U('del',array('life_id'=>$vo['life_id']));?>" class="btn btn-danger">
